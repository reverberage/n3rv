"""Tests for n3rverberage update CLI command.
...
def test_update_skips_command_files_by_default(initialized_project: Path):
    ...
    # n3rverberage uses SKIP_DEFAULT for command files — no separate .claude path
    pass
...
def test_update_force_commands_overwrites_command_files(initialized_project: Path):
    ...
    # n3rverberage uses SKIP_DEFAULT with --force-commands override
    pass
...
def test_update_warns_on_missing_end_marker(initialized_project: Path, capsys):
    ...
    # n3rverberage uses overwrite strategy for infra files, not marker_merge
