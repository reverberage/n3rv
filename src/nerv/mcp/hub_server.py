...
from nerv.mcp.shared import build_mcp_server, detect_agent_source, hub_rpc, resolve_runtime_settings, result_payload
...
def _list_pending_tasks(hub_url: str, agent_id: str | None = None) -> list[dict]:
    ...
    return sorted(tasks_by_id.values(), key=lambda task: task.get("status", {}).get("timestamp", ""))
...
def build_hub_server(project_root: Path | None = None):
    ...
    async def delegate_task(
        ...
        result = _rpc(hub_url, "tasks/send", {
            "skill_id": skill_id,
            "description": description,
            "requesting_agent": requesting_agent,
            "metadata": {"auto_complete": False},
        })
        logger.info("delegate_task skill=%s from=%s -> task=%s", skill_id, requesting_agent, result.get("id"))
    ...
    async def list_pending_tasks(agent_id: str | None = None) -> list:
        ...
        logger.info("list_pending_tasks agent=%s -> %d tasks", agent_id or detect_agent_source(), len(tasks))
    ...
    async def check_pending_tasks(agent_id: str | None = None) -> list:
        tasks = _list_pending_tasks(hub_url, agent_id)
        logger.info("check_pending_tasks agent=%s -> %d tasks", agent_id or detect_agent_source(), len(tasks))
        return result_payload(tasks)
    ...
    async def complete_task(task_id: str, result: str, completing_agent: str = "unknown") -> dict:
        out = _rpc(hub_url, "tasks/complete", {
            "task_id": task_id,
            "result": result,
