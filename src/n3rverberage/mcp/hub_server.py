...
from n3rverberage.util import get_hub_url

logger = logging.getLogger("n3rverberage.mcp.hub")
...
def _resolve_agent_ids(agent_id: str | None = None) -> list[str]:
    ...
    if not resolved:
        raise ValueError("agent_id is required when N3RVERBERAGE_AGENT_SOURCE is not set")
...
def build_hub_server(project_root: Path | None = None):
    ...
    @server.tool(
        description=(
            ...
            "If agent_id is omitted, uses the current agent from N3RVERBERAGE_AGENT_SOURCE."
        )
    )
    ...
    @server.tool(
        description=(
            ...
            "Pass agent_id explicitly when N3RVERBERAGE_AGENT_SOURCE is not set."
        )
    )
