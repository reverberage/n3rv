...
from n3rverberage.mcp.shared import resolve_runtime_settings
...
def _unit_path(root: Path) -> Path:
    return root / ".n3rverberage" / "systemd" / "n3rverberage-hub.service"
...
def daemon_install(root: Path) -> int:
    ...
    if not src.exists():
        ...
        print("Run 'n3rverberage init' first", file=sys.stderr)
        return 1
    dst_dir = Path.home() / ".config" / "systemd" / "user"
    dst_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy2(src, dst_dir / "n3rverberage-hub.service")
    result = _systemctl("daemon-reload")
    if result.returncode != 0:
        print(f"Error: daemon-reload failed: {result.stderr}", file=sys.stderr)
        return 1
    print("Hub daemon installed. Run 'n3rverberage daemon enable --now' to enable and start.")
    return 0


def daemon_start() -> int:
    _check_systemd()
    result = _systemctl("start", "n3rverberage-hub")
    if result.returncode != 0:
...
def daemon_stop() -> int:
    _check_systemd()
    result = _systemctl("stop", "n3rverberage-hub")
    if result.returncode != 0:
        print(f"Error: {result.stderr.strip()}", file=sys.stderr)
        return 1
...
def daemon_status() -> int:
    _check_systemd()
    result = _systemctl("is-active", "n3rverberage-hub")
...
def daemon_logs(root: Path) -> int:
    ...
    if log_file.exists():
        ...
        return 0
    result = _systemctl("status", "n3rverberage-hub")
    print(result.stdout)
    return 0
