...
from n3rverberage.a2a.router import RoutingDecision, SkillNotFoundError, TaskRouter
from n3rverberage.a2a.state import DelegationArtifact, HubStateStore
from n3rverberage.config import RuntimeSettings
from n3rverberage.init.registry import SkillRegistry
from n3rverberage.mcp.memory_service import MemoryService
from n3rverberage.mcp.shared import ensure_runtime_directories, resolve_runtime_settings
from n3rverberage.models.a2a import TaskState

logger = logging.getLogger("n3rverberage.hub")
...
async def run_hub() -> None:
    ...
    await site.start()
    logger.info("n3rverberage hub listening on %s:%d", settings.a2a_host, settings.a2a_port)
    ...
    try:
        ...
        logger.info("n3rverberage hub shutting down on signal")
    finally:
        _mark_working_tasks_failed(hub)
        await runner.cleanup()
        logger.info("n3rverberage hub stopped")
        try:
...
def main() -> None:
    log_level = os.environ.get("N3RVERBERAGE_LOG_LEVEL", "INFO").upper()
